...
# envia uma mensagem para uma única pessoa
...
def enviar_mensagem_individual(conexao):
    ...
    for i in range(conexao['last'], len(mensagens)):    #percorre todos os elementos
        ...
        conexao['conn'].send(mensagem_de_envio.encode()) #envia a mensagem de acordo com a conexão conn do cliente estabelecido
        conexao['last'] = i + 1                         #incrementa o número da conexão
...
start()   # inicia a função start que permanece no laço while(True)